...
- ✏️ **演员数据编辑**: 支持编辑演员名称，添加曾用名，修改头像（需外挂数据文件），当识别到重复名称时可合并（本功能只修改数据库，而不会修改原始nfo文件中的信息，请放心使用，如需编辑NFO中的信息请使用详情页的编辑功能）
- 🔎 **高级搜索**: 提供简易搜索和多重搜索两种模式，支持多条件组合查询
- ❤️ **收藏夹**: 图文列表页、影片详情页可点击收藏，支持多收藏夹
- 🧩 **分类管理**: 分类 Tab 支持编辑当前分类列表与一键导入默认分类，编辑弹窗中每个小类右侧展示「可播放/总数」统计，便于快速了解当前配置覆盖情况
...
若需在应用中显示演员头像，可拉取 [gfriends/gfriends](https://github.com/gfriends/gfriends) 项目；在设置中选择的演员数据路径**必须包含** `Filetree.json` 与 `Content` 目录。选好包含以上两者的文件夹路径后点击「扫描演员信息」即可。
若需自建或修改数据，请参阅 [演员头像数据格式说明（Filetree.json + Content）](演员头像数据格式说明.md)。

**注意：** 考虑到演员数据一般较多，所以对该页面进行了缓存处理，不会频繁刷新，如果添加了演员数据发现头像并没有出现，可点击页面上的刷新按钮进行刷新

### 主要功能

- **影片详情**: 点击影片进入详情页，可查看完整信息、编辑数据、播放视频；演员名称统一优先使用用户编辑后的显示名称，未设置显示名时回退为 NFO 中的原始名称
- **收藏夹管理**: 图文列表页及详情页可对影片进行收藏，支持自定义收藏夹
- **目录浏览**: 在"目录"页面通过下拉框切换不同类型的目录（演员、文件目录、导演、制作商、分类）；演员目录下支持放大/缩小按钮调整网格卡片大小，支持显示头像与统计信息
- **搜索功能**: 在"搜索"页面使用简易搜索或多重搜索查找影片；简易搜索支持输入即搜（防抖 0.7 秒），结果在页内展示
- **设置管理**: 在"设置"页面配置影片/演员数据路径、扫描数据、调整显示和同步设置
- **演员目录与头像**: 可选配置演员数据路径后，演员列表页（仅演员模式）、详情页、目录页展示头像；未配置时演员列表页不显示头部区域，配置后无头像时显示占位保持布局；支持简繁体与曾用名匹配，多头像时可选择默认展示
- **演员名称与曾用名**: 演员列表页（演员模式）名称旁可编辑显示名称与曾用名（仅改应用库，不修改 NFO）；曾用名参与「扫描演员信息」时的头像匹配；编辑显示名或曾用名与其他演员发生重名/曾用名冲突时，系统会提示是否合并为同一演员，并采用软合并方式统一其影片与别名（不回写 NFO）
